...
class ResumeCreateView(BaseUserView):
    ...
    def post(self, request):
        try:
            ...
            careers_data = (
                [data.get("career")]
                if "," not in data.get("career")
                else data.get("career").split(",")
            )
            awards_data = (
                [data.get("award")]
                if "," not in data.get("award")
                else data.get("award").split(",")
            )
            completions_data = (
                [data.get("completion")]
                if "," not in data.get("completion")
                else data.get("completion").split(",")
            )
            regions_data = (
                [data.get("region")]
                if "," not in data.get("region")
                else data.get("region").split(",")
            )

            Education.objects.bulk_create(
                [
                    Education(education=education.strip(), resume=resume)
            ...
            Career.objects.bulk_create(
                [
                    Career(career=career.strip(), resume=resume)
            ...
            Completion.objects.bulk_create(
                [
                    Completion(completion=completion.strip(), resume=resume)
            ...
            Region.objects.bulk_create(
                [
                    Region(region=region.strip(), resume=resume)
