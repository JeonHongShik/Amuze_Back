...
class Completion(models.Model):
    ...
    def __str__(self):
        return f"{self.resume.author}님이 작성한 게시글 {self.completion} 수료"
...
class Resume(models.Model):
    ...
    mainimage = models.FileField(
        upload_to="image/resume/%Y/%m/%d/", null=True, blank=True
    )
    otherimages1 = models.FileField(
        upload_to="image/resume/%Y/%m/%d/", null=True, blank=True
    )
    otherimages2 = models.FileField(
        upload_to="image/resume/%Y/%m/%d/", null=True, blank=True
    )
    otherimages3 = models.FileField(
        upload_to="image/resume/%Y/%m/%d/", null=True, blank=True
    )
    otherimages4 = models.FileField(
        upload_to="image/resume/%Y/%m/%d/", null=True, blank=True
    )
    ...
    def save(self, *args, **kwargs):
        if self.pk:
            ...
            if (
                original_resume.mainimage
                and original_resume.mainimage.name != self.mainimage.name
            ):
                storage.delete(original_resume.mainimage.name)
            if (
                original_resume.otherimages1
                and original_resume.otherimages1.name != self.otherimages1.name
            ):
                storage.delete(original_resume.otherimages1.name)
            if (
                original_resume.otherimages2
                and original_resume.otherimages2.name != self.otherimages2.name
            ):
                storage.delete(original_resume.otherimages2.name)
            if (
                original_resume.otherimages3
                and original_resume.otherimages3.name != self.otherimages3.name
            ):
                storage.delete(original_resume.otherimages3.name)
            if (
                original_resume.otherimages4
                and original_resume.otherimages4.name != self.otherimages4.name
            ):
